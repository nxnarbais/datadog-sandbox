...
resource "datadog_monitor" "usage_ingested_log_index_quota" {
  type                = "metric alert"
...
EOF
  ...
  include_tags        = true
  priority            = 3
  tags                = concat(["standard:true", "terraform:true", "type:usage_monitor", "sku:ingested_logs", "owner:${var.owner}"], var.tags)
}
...
resource "datadog_monitor" "usage_ingested_logs_anomalies" {
  type                = "metric alert"
  name                = "[Usage][IngestedLogs][ForComposite] Anomaly on {{service.name}}"
  query               = <<EOF
avg(last_1d):anomalies(sum:datadog.estimated_usage.logs.ingested_bytes{!datadog_index:some_index_to_exclude} by {service}.as_count(), 'agile', 3, direction='above', interval=300, alert_window='last_1h', count_default_zero='true', seasonality='weekly') >= 1
EOF
  ...
  include_tags        = true
  priority            = 5
  tags                = concat(["composite:true", "standard:true", "terraform:true", "type:usage_monitor", "sku:ingested_logs", "owner:${var.owner}"], var.tags)
}

resource "datadog_monitor" "usage_ingested_logs_minimum_volume" {
  type                = "metric alert"
  name                = "[Usage][IngestedLogs][ForComposite] Minimum ingested log volume"
  query               = <<EOF
sum(last_15m):sum:datadog.estimated_usage.logs.ingested_bytes{!datadog_index:some_index_to_exclude} by {service}.as_count() < ${var.minimum_service_volume}
EOF
  message = "N/A - Used with composite monitor to reduce alert fatigue"
  monitor_thresholds {
...
resource "datadog_monitor" "usage_ingested_logs_anomalies_with_minimum_threshold" {
  name    = "[Usage][IngestedLogs] Anomaly on {{service.name}} (with minimum threshold)"
  type    = "composite"
  query   = "${datadog_monitor.usage_ingested_logs_anomalies.id} && !${datadog_monitor.usage_ingested_logs_minimum_volume.id}"
...
resource "datadog_monitor" "usage_ingested_logs_anomalies_by_context" {
  type                = "metric alert"
  name                = "[Usage][IngestedLogs] Anomaly on specific context"
  query               = <<EOF
avg(last_1d):anomalies(sum:datadog.estimated_usage.logs.ingested_bytes{!datadog_index:some_index_to_exclude,${var.context_filter}} by {${var.by_tag_keys}}.as_count(), 'agile', 3, direction='above', interval=300, alert_window='last_1h', count_default_zero='true', seasonality='weekly') >= 1
EOF
