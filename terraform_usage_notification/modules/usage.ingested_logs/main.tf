...
resource "datadog_monitor" "usage_ingested_logs_anomalies" {
  ...
  query               = <<EOF
avg(last_1d):anomalies(sum:datadog.estimated_usage.logs.ingested_bytes{!datadog_index:some_index_to_exclude} by {service}.as_count(), 'agile', 5, direction='above', interval=300, alert_window='last_1h', count_default_zero='true', seasonality='weekly') >= 1
EOF
